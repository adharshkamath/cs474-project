# reproduce starts
Fixes some bug for a specific run on a server. 
Quick Setup: In a python3.10.9 environment
```
...
run:
```
bash ddar.sh # results in ddar.txt
bash alphageometry.sh $ results in alphageometry.txt
```
Problems: Meliad has unstable output (hardware dependent) causing the lm_inference_test in run_tests.sh to fail. 

Find: DDAR has no " * Auxiliary Constructions:", while alphageometry does. 
