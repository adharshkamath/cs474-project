...
(assert
 (let (($x645 (= (Segment F1 D1) (Segment D1 A1))))
 (=> |Segment(F1, D1) == Segment(D1, A1)| $x645)))
(assert
 (let (($x646 (On F1 FD1)))
 (=> |On(F1, FD1)| $x646)))
(assert
...
(assert
 ...
 (=> |On(A1, FD1)| $x649)))
(assert
 (let (($x824 (= (Segment G1 E1) (Segment E1 A1))))
 (=> |Segment(G1, E1) == Segment(E1, A1)| $x824)))
(assert
 (let (($x825 (On G1 GE1)))
 (=> |On(G1, GE1)| $x825)))
(assert
...
(assert
 ...
 (=> |On(A1, GE1)| $x827)))
(assert
 (let (($x830 (Center I1 OABC)))
 (=> |Center(I1, OABC)| $x830)))
(assert
 (let (($x829 (Onc A1 OABC)))
 (=> |Onc(A1, OABC)| $x829)))
(assert
 (let (($x831 (Onc B1 OABC)))
 (=> |Onc(B1, OABC)| $x831)))
(assert
 (let (($x832 (Onc C1 OABC)))
(=> |Onc(C1, OABC)| $x832)))
(check-sat)
