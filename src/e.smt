...
;
; Test diagram
; a b = segment a b; c = mirror c a b; d = mirror d b c; e = midpoint e c b; 
; f = on_circle f e c, on_dia f a e; g = on_line g a f ? eqangle b f a f c f e f
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
:assumption (on B AB)
:assumption (on C AB)
:assumption (bet C B A)
:assumption (= (seg C B) (seg B A))

:assumption (on D AB)
:assumption (bet D C B)
:assumption (= (seg D C) (seg C B))

:assumption (on E AB)
:assumption (bet C E B)
:assumption (= (seg E B) (seg C E))

:assumption (center E ECF)
:assumption (onc C ECF)
:assumption (onc F ECF)

:assumption (on A AF)
:assumption (on F AF)
:assumption (= (angle A F E) rightangle)

:extrafuns ((M Point) (DiaAE Circle))
...
:formula (= (angle B F A) (angle C F E))
...
; :assumption (bet A B M)
; :assumption (= (seg M B) (seg M A))
